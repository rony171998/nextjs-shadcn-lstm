...
export default async function Page({ params }: { params: { slug: string } }) {
    ...
    try {
      const response = await axios.get<{ success: boolean; data: KlineData[]; error?: string }>(
        `${baseUrl}/api/klines`,
        {
          params: {
            symbol,
      ...
      if (!response.data.success) {
        throw new Error(response.data.error || 'La respuesta de la API no fue exitosa');
      }

      klines = response.data.data || [];
    } catch (error) {
